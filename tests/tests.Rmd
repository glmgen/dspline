...
matplot(xd, H, type = "p", pch = 19)
matplot(x, Hx, type = "l", lty = 1, add = TRUE)
```
...
for (j in 1:ncol(N4)) N3[,j] = dspline_interp(N1[,j], k, xd, x)
max(abs(N2 - N3))

ylim = range(N2[min(xd) <= x & x <= max(xd), ])
matplot(x, N2, type = "l", lty = 1, ylim = ylim)
matplot(xd, N1, type = "p", pch = 19, add = TRUE)
```
...
n = 500
k = 2
knot_idx = sort(sample((k+1):(n-1), 100))
xd = sort(runif(n))
v = rnorm(n)
...
max(abs(obj2$sol - obj3$sol)) / sum(abs(obj2$sol))
sum((v - obj1$fit)^2) - sum((v - obj2$fit)^2)
sum((v - obj1$fit)^2) - sum((v - obj3$fit)^2)
```